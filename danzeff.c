...
char modeChar[MODE_COUNT*2][3][3][5] =
{
	...
	{	//numbers
		...
		{ "\0\0\0004",  "\010\007 5","\0\0\0006" },	// MIB.42_4 : Use #07 to signal backspace
		{ "\260\047\0427","\0\0\0008", "\0\00009" }	// MIB.42_4 : Add °'"
	},
...
void readKeyboardSettings() {
	...
	char *ptr,cc;
        while(fgets(line,128,fp) != NULL) {
                if (line[0]=='#')
                        continue;
                if (strncmp(line,"STANDARD_LETTERS=",17)==0) {
			ptr=strtok(&line[17],"|");
			for (j=0; j<9;j++) {
				for (i=0; i<4;i++) {
					if (ptr!=NULL && strlen(ptr)!=0) {
						if (strlen(ptr)==1)
							...
							cc=atoi(ptr);
						//fprintf(stdout, "[%c][%d]::[%c][%d]\n", modeChar[0][0][j][i],  modeChar[0][0][j][i],cc,cc);
						modeChar[0][0][j][i]=cc;
						ptr=strtok(NULL,"|");
					}
				}
                ...
                if (strncmp(line,"CAPITAL_LETTERS=",16)==0) {
			ptr=strtok(&line[16],"|");
			for (j=0; j<9;j++) {
				for (i=0; i<4;i++) {
					if (ptr!=NULL && strlen(ptr)!=0) {
						if (strlen(ptr)==1)
							...
							cc=atoi(ptr);
						//fprintf(stdout, "[%c][%d]::[%c][%d]\n", modeChar[1][0][j][i],  modeChar[1][0][j][i],cc,cc);
						modeChar[1][0][j][i]=cc;
						ptr=strtok(NULL,"|");
					}
				}
			...
			for (j=0; j<9;j++) {
				for (i=0; i<4;i++) {
					if (ptr!=NULL && strlen(ptr)!=0) {
						if (strlen(ptr)==1)
							...
							cc=atoi(ptr);
						//fprintf(stdout, "[%c][%d]::[%c][%d]\n", modeChar[2][0][j][i],  modeChar[2][0][j][i],cc,cc);
						modeChar[2][0][j][i]=cc;
						ptr=strtok(NULL,"|");
					}
				}
                ...
                if (strncmp(line,"SPECIAL_CHARACTERS=",19)==0) {
			ptr=strtok(&line[19],"|");
			for (j=0; j<9;j++) {
				for (i=0; i<4;i++) {
					if (ptr!=NULL && strlen(ptr)!=0) {
						if (strlen(ptr)==1)
							...
							cc=atoi(ptr);
						//fprintf(stdout, "[%c][%d]::[%c][%d]\n", modeChar[3][0][j][i],  modeChar[3][0][j][i],cc,cc);
						modeChar[3][0][j][i]=cc;
						ptr=strtok(NULL,"|");
					}
				}
...
u32 danzeff_convert_pow2(u32 size)
{
	...
	for ( ; pow_counter < 32 ; pow_counter++)
	{
		...
		if ((size >> pow_counter) == 0)
		{
			...
			if ((1 << pow_counter) !=(signed int) size )
			{
	...
	for (a = 0; a < guiStringsSize; a++)
	{
		...
		if (danzeff_get_png_image_size(guiStrings[a], &width, &height) == 0)
		{
			...
			else
			{
				...
				keyTextures[a].texture_width 	= (u32)width;
				keyTextures[a].texture_height 	= (u32)height;
				keyTextures[a].surface_width 	= (u32)danzeff_convert_pow2(width);
				keyTextures[a].surface_height 	= (u32)danzeff_convert_pow2(height);
